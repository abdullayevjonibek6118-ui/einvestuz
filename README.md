...
- AI-чат с контекстом доступных рыночных данных;
...
| `/ai` | AI-анализ | Чат для образовательных вопросов о компаниях, показателях и рисках; справа отображается рыночный контекст по доступным акциям. Можно передать стартовый вопрос через `?question=...`. |
| `/portfolio` | Виртуальный портфель | Добавление и удаление учебных позиций, расчёт стоимости, прибыли/убытка и доходности. Позицию можно предварительно выбрать через `?ticker=...`. |
| `/profile` | Профиль | Демонстрационный watchlist, настройки уведомлений и история AI-запросов. Страница закрыта от поисковой индексации. |
...
- виртуальный портфель сохраняется в `localStorage` текущего браузера; backend-операции с портфелем используют память процесса и не заменяют полноценный пользовательский кабинет;
- профиль, watchlist, переключатели уведомлений и история запросов пока демонстрационные;
- `/chat` возвращает образовательный ответ на основе MVP-логики, а не персональную инвестиционную рекомендацию;
- при недоступности внешнего провайдера интерфейс использует fallback-данные;
- `yfinance` — best-effort источник без SLA, а публичные UZSE/StockScope-данные могут поступать с задержкой;
- макроисточники `data.egov.uz` и `stat.uz` частично представлены метаданными и заглушками до подтверждения стабильных машиночитаемых endpoints;
...
| Метод | Endpoint | Назначение |
| --- | --- | --- |
| `POST` | `/portfolio/add` | Добавить виртуальную позицию в память backend. |
| `POST` | `/portfolio/remove` | Удалить виртуальную позицию. |
| `POST` | `/newsletter` | Провалидировать и сохранить email в памяти процесса. |
| `POST` | `/chat` | Получить образовательный ответ AI-помощника MVP. |
