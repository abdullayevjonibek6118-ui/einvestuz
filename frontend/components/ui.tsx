...
export function PageHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="mb-5 flex flex-col gap-3 border-b border-[#dbe4ef] pb-5 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="inline-flex w-fit items-center gap-2 rounded-lg border border-[#fed7aa] bg-[#fff7ed] px-3 py-2 text-xs font-medium text-[#9a3412]">
...
export function Panel({
  ...
  return (
    <section className={`rounded-2xl border border-[#dbe4ef] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)] ${className}`}>
      {(title || action) && (
...
export function ChangeBadge({ value }: { value: number }) {
  ...
  return (
    <span className={`tabular-data inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold ${positive ? "bg-[#dcfce7] text-[#15803d]" : "bg-[#fee2e2] text-[#b91c1c]"}`}>
      <Icon size={13} />
      {positive ? "+" : ""}
      {value.toFixed(2)}%
    </span>
...
export function Metric({ label, value, detail }: { label: string; value: string; detail?: string }) {
  return (
    <div className="rounded-2xl border border-[#dbe4ef] bg-[#f8fafc] p-3 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
